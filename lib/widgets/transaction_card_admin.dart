...
import '../screens/transaction_details_screen_.dart';

class TransactionCardAdmin extends StatelessWidget {
  final TransactionModel transaction;

  const TransactionCardAdmin({super.key, required this.transaction});
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => TransactionDetailsScreenTwo(transaction: transaction),
          ),
        );
      },
      child: Card(
        ...
        child: ListTile(
          ...
          subtitle: Column(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () async {
                  await Clipboard.setData(ClipboardData(text: "${transaction.sendAmount} ➝ ${transaction.receiveAmount}"));
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text("تم نسخ المبلغ: ${transaction.sendAmount} ➝ ${transaction.receiveAmount}"),
              ...
              GestureDetector(
                onTap: () async {
                  await Clipboard.setData(ClipboardData(text: transaction.transactionNumber));
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text("تم نسخ رقم العملية: ${transaction.transactionNumber}"),
                      backgroundColor: Colors.green,
                    ),
                  );
                },
